...
| Hugging Face, Inc. (US) / Hugging Face SAS (FR) | AI model CDN — up to two model downloads on first start, depending on deployment mode: (1) GLiNER named-entity detection model (~220MB) — downloaded when the anonymizer sidecar starts; (2) BGE-small-en intent embedding model (~130MB, downloaded to `~/.zemtik/models/`) — downloaded when the proxy starts with the default `embed` intent backend (`ZEMTIK_INTENT_BACKEND=embed`); not downloaded in regex-only deployments (`ZEMTIK_INTENT_BACKEND=regex`). Both models run locally after download; no data is sent to Hugging Face during operation. | United States / France | Yes — [huggingface.co/privacy](https://huggingface.co/privacy) |
