...
**Scope note:** This document is aligned with **v0.14.0** (see `CHANGELOG.md`). The ZK slow-lane cryptography is unchanged in spirit from earlier releases.
...
- `type_hash` — 4-hex code = `hex(SHA-256(entity_type)[0..2])`. Hardcoded in `src/entity_hashes.rs` (23 types). The Python sidecar uses `sidecar/entity_hashes.py` (canonical) and `sidecar/zemtik_entity_hashes.py` (re-export shim used by `server.py`) — all three must agree.
...
A fresh in-memory vault is created per request. `scopeguard::defer!` clears it immediately after the response is returned. A background task evicts any vault older than `ZEMTIK_ANONYMIZER_VAULT_TTL_SECS` (default 300s). The vault is never written to disk.
...
| Proxy API key | `ZEMTIK_PROXY_API_KEY` required; gates all inbound requests |
| ZK proof verification | `bb verify` with configurable timeout (`ZEMTIK_VERIFY_TIMEOUT_SECS`) |
| PII anonymization | Sidecar gRPC + regex fallback; vault cleared per-request + TTL eviction |
...
The following items are stable across patch and minor releases of `zemtik-core`. They are re-exported from the crate root and documented via `cargo doc --no-deps`.
...
Items marked `#[doc(hidden)]` are **internal** and may change without notice in any release. Do not depend on them from external crates.
...
- **Minor** (`0.x → 0.x+1`): additive changes to the stable surface. New fields on `#[non_exhaustive]` structs, new enum variants on non-exhaustive enums, new re-exports. Existing stable items unchanged.
- **Major** (`0.x → 1.0`): breaking changes to the stable surface. Will include migration notes.

## Transitive Stable Dependencies

`build_proxy_router` returns `axum::Router`, making `axum` an effective part of the semver contract. The crate targets `axum 0.7.x`. If axum introduces breaking changes we will bump our minor version and document the required axum version in the changelog.
