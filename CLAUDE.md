...
- **`zemtik_mode` request field (v0.15.4+):** Clients can add `"zemtik_mode": "document"` to the top-level request body to skip intent matching entirely and force `general_lane`. Requires `ZEMTIK_GENERAL_PASSTHROUGH=1`. Returns 400 if passthrough is disabled. `"zemtik_mode": "data"` forces normal routing. Ignored in tunnel mode. The field is stripped before forwarding to OpenAI.
- **Document-only deployments:** Use `schema_config.legal.json` (repo root) — an empty `tables` map that forces all requests to `general_lane` (with passthrough enabled) while keeping the anonymizer pipeline active. Mount via `ZEMTIK_SCHEMA_CONFIG_PATH=./schema_config.legal.json` or Docker volume: `-v ./schema_config.legal.json:/root/.zemtik/schema_config.json`.
